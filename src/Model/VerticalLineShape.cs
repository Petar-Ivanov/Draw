...
{
    /// <summary>
    /// Класът вертикална линия е основен примитив, който е наследник на базовия Shape.
    /// </summary>
    [Serializable]
    public class VerticalLineShape : Shape
	{
        /// <summary>
        ...
        public static int VerticalLineCounter = 1;

        #region Constructor

        public VerticalLineShape(RectangleF rect) : base(rect)
		{
            this.Name = $"VerticalLine{VerticalLineCounter++}";
        }
		
		public VerticalLineShape(RectangleShape rectangle) : base(rectangle)
		{
            this.Name = $"VerticalLine{VerticalLineCounter++}";
        }
        ...
        /// <summary>
		/// Проверява за принадлежност на точка p към вертикалната линия.
        ...
        {
            VerticalLineShape copy = new VerticalLineShape(new RectangleF(Rectangle.X, Rectangle.Y, Rectangle.Width, Rectangle.Height));
            copy.Name = Name;
        ...
        public override void DrawSelf(Graphics grfx)
		{
            ...
            grfx.DrawLine(new Pen(Color.FromArgb(BorderColorOpacity, BorderColor), BorderRadius), new PointF(Location.X, Location.Y), new PointF(Location.X, Location.Y + Rectangle.Height));

            PointF[] verticalLinePoints = new PointF[]
            {
                new PointF(Location.X - (float)(BorderRadius*0.5), Location.Y), 
				new PointF(Location.X - (float)(BorderRadius*0.5), Location.Y + Rectangle.Height),
                new PointF(Location.X + (float)(BorderRadius*0.5), Location.Y + Rectangle.Height),
                new PointF(Location.X + (float)(BorderRadius*0.5), Location.Y)
            };

            Vertices = verticalLinePoints;
